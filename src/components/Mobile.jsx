...
const Mobile = () => {
  ...
  useEffect(() => {
    const fetchMobile = async () => {
      try {
        ...
        console.error("Error loading mobiles data:", err);
        setLoading(false);
      }
    };
    fetchMobile();
  ...
  useEffect(() => {
    if (mobileData.length > 0) {
      const prices = mobileData.map((item) => item.price);
  ...
  // Get unique list of brands from the mobile data
  const brands = [
    ...new Set(mobileData.map((item) => item.name.split(" ")[0])),
  ].sort();
  ...
  const filteredMobiles = mobileData.filter((item) => {
    ...
    if (selectedProcessors.length > 0 && !selectedProcessors.includes(procCat))
      return false;
    ...
    if (selectedStorage.length > 0 && !selectedStorage.includes(storageSize))
      return false;
  ...
  const getBrandCount = (brand) => {
    return mobileData.filter((item) => item.name.split(" ")[0] === brand)
      .length;
  };

  const getProcessorCount = (procCat) => {
    return mobileData.filter(
      (item) => getProcessorCategory(item.specs?.processor) === procCat,
    ).length;
  };
      ...
      <div className="mb-6 sm:mb-8 border-b border-slate-100 pb-4 sm:pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-3 sm:gap-4">
        <div>
          ...
          <p className="font-medium text-slate-500 text-sm sm:text-base mt-1">
            Experience next-generation mobile silicon, advanced camera systems,
            and premium titanium designs.
          </p>
        </div>
        <div className="text-sm font-semibold text-slate-600 bg-slate-100 px-4 py-2 rounded-full shadow-2xs">
          Showing {filteredMobiles.length} of {mobileData.length} Devices
        </div>
      </div>
            ...
            {filteredMobiles.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-20 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl p-8">
                ...
                <p className="text-slate-500 text-sm max-w-sm mb-6">
                  ...
                  clearing all filters to browse our full selection.
                </p>
                <button
                  onClick={handleClearAll}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                {filteredMobiles.map((items) => {
                  const { id, name, price, description, image, specs } = items;

                  return (
                    <div
                      key={id}
                      className="group bg-white border border-slate-100 rounded-3xl shadow-xs hover:shadow-xl hover:-translate-y-1.5 transition-all duration-300 flex flex-col justify-between overflow-hidden min-h-[400px] sm:min-h-[440px]"
                    >
                      {/* Image container */}
                      <Link
                        to={`/product/${items.category || "mobile"}/${id}`}
                        className="relative aspect-video w-full overflow-hidden bg-slate-50 block"
                      >
                        <img
                            ...
                            {specs?.processor && (
                              <span className="text-[10px] font-bold bg-slate-50 border border-slate-100 text-slate-600 px-2.5 py-1 rounded-lg flex items-center gap-1">
                                ...
                                {specs.processor
                                  .split("(")[0]
                                  .trim()
                                  .replace("10-core CPU, 10-core GPU", "")
                                  .trim()}
                              </span>
                            )}
                            {specs?.ram && (
