...
const Mobile = () => {
  ...
  const brands = [...new Set(mobileData.map((item) => item.name.split(" ")[0]))].sort();
  ...
  const getBrandCount = (brand) => {
    return mobileData.filter((item) => item.name.split(" ")[0] === brand).length;
  };

  const getProcessorCount = (procCat) => {
    return mobileData.filter((item) => getProcessorCategory(item.specs?.processor) === procCat).length;
  };
      ...
      <div className="mb-8 border-b border-slate-100 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="font-medium text-slate-500 text-base mt-1">
            Experience next-generation mobile silicon, advanced camera systems, and premium titanium designs.
          </p>
        </div>
          ...
          <div className="lg:col-span-3">
            {filteredMobiles.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-20 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl p-8">
                ...
                <h3 className="text-lg font-bold text-slate-900 mb-1">No devices match your criteria</h3>
