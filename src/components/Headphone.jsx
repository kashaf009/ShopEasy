...
const Headphone = () => {
  ...
  useEffect(() => {
    if (audioDevices.length > 0) {
      const prices = audioDevices.map((item) => item.price);
  ...
  // Get unique lists
  const brands = [...new Set(audioDevices.map((item) => item.name.split(" ")[0]))].sort();
  const categories = ["headphones", "earbuds"];
  const ancOptions = ["Yes", "No"];
  ...
  const handleAncToggle = (anc) => {
    setSelectedAnc((prev) =>
      prev.includes(anc) ? prev.filter((a) => a !== anc) : [...prev, anc]
    );
  };
  ...
  // Apply filtering
  const filteredAudio = audioDevices.filter((item) => {
    // Brand
    ...
    // ANC
    const hasAnc = item.specs?.anc === "Yes" ? "Yes" : "No";
    if (selectedAnc.length > 0 && !selectedAnc.includes(hasAnc)) return false;

    return true;
  });

  const getBrandCount = (brand) => audioDevices.filter((item) => item.name.split(" ")[0] === brand).length;
  const getCategoryCount = (cat) => audioDevices.filter((item) => item.category === cat).length;
  const getAncCount = (anc) => audioDevices.filter((item) => (item.specs?.anc === "Yes" ? "Yes" : "No") === anc).length;
        ...
        <>
          ...
          >
            ...
            <div className="space-y-6">
              ...
              {/* Category Filter */}
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Type</h4>
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {categories.map((cat) => {
                      ...
                      <label key={cat} className="flex items-center justify-between cursor-pointer group text-sm">
                        <div className="flex items-center gap-2.5">
                          <input
                            ...
                            className="w-4 h-4 rounded border-slate-300 text-slate-900 focus:ring-slate-900 focus:ring-offset-0 accent-slate-900"
                          />
                          <span className="text-slate-700 group-hover:text-slate-950 font-medium transition-colors capitalize">
                            {cat}
                          </span>
                        </div>
                        <span className="text-xs text-slate-400 font-semibold bg-slate-50 px-2 py-0.5 rounded-md">
              ...
              {/* ANC Filter */}
              <div>
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Active Noise Cancellation</h4>
                <div className="flex flex-wrap gap-2">
                  {ancOptions.map((anc) => {
                    const count = getAncCount(anc);
                    if (count === 0) return null;
                    const isActive = selectedAnc.includes(anc);
                    return (
                      <button
                        key={anc}
                        onClick={() => handleAncToggle(anc)}
          ...
          <div className="lg:col-span-3">
            {filteredAudio.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-20 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl p-8">
                ...
                <h3 className="text-lg font-bold text-slate-900 mb-1">No audio devices match your criteria</h3>
