...
const NewArrivalCard = ({
  ...
  return (
    id && (
      <div
        key={id}
        className="shadow-md cursor-pointer hover:shadow-xl transition-all"
      ...
      >
        ...
        <div className="flex justify-between ">
          <Link to={`/product/${category || "new"}/${id}`}>
        ...
        {specs && (
          <div className="flex flex-wrap gap-1.5 px-5 mb-3">
            {Object.entries(specs).map(([key, value]) => (
              <span
                key={key}
                className="text-[10px] font-bold bg-slate-50 border border-slate-100 text-slate-600 px-2.5 py-1 rounded-lg flex items-center gap-1"
              >
